...
import { resolveEntitySchema, resolveSchema } from "../schemas";
  ...
  for (const [type, collection] of Object.entries(collections) as [
    ...
    for (const [key, entity] of Object.entries(records)) {
      ...
      writeJson(`${entityDirectory}.json`, {
        ...
        entity,
        effectiveSchema:
          type === "event" || type === "attribute"
            ? resolveEntitySchema(entity, catalog.entities.schemas)
            : type === "schema"
              ? resolveSchema(entity, catalog.entities.schemas)
              : undefined,
...
export async function exportCatalog(deps: Dependencies) {
  ...
  if (fs.existsSync(outputRoot)) {
    const entries = fs.readdirSync(outputRoot).filter((entry) => entry !== ".DS_Store");
    const wouldReplaceExistingContent = copyAssets
      ? entries.length > 0
      : ["data", "catalog.json", "catalog-manifest.json", "history-full.json"].some((entry) =>
          fs.existsSync(path.join(outputRoot, entry)),
        );
    if (wouldReplaceExistingContent && !fs.existsSync(path.join(outputRoot, CATALOG_MARKER))) {
      throw new Error(
        `Refusing to clean Catalog output directory "${outputRoot}" because it was not generated by Eventvisor.`,
      );
    }
  }
