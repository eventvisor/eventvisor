...
Key facts that prevent most integration mistakes: the app must load a **datafile** (built and deployed from the project repo) and decide its own refresh strategy; `track`/`setAttribute`/`setDatafile`/`close` are **async** — await them when ordering matters; `track()` resolving to `null` means the pipeline dropped the event (see the pipeline above); event keys, property names, and attribute names must match the project's definitions exactly — verify against the project (or its Catalog) rather than guessing; `await eventvisor.onReady()` is only needed when persistence modules are in play.
